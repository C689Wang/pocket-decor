...
private let logger = Logger(subsystem: "com.apple.pocketDecorObjectCapture",
                            category: "ObjectCaptureApp")

/// Implements the main command structure, defines the command-line arguments,
/// and specifies the main run loop.
struct ObjectCaptureApp: ParsableCommand {
                ...
                for try await output in session.outputs {
                    switch output {
                        ...
                        case .requestComplete(let request, let result):
                            ObjectCaptureApp.handleRequestComplete(request: request, result: result)
                        case .requestProgress(let request, let fractionComplete):
                            ObjectCaptureApp.handleRequestProgress(request: request,
